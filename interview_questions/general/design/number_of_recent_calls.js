...
in the past 3000 milliseconds (including the new request). Specifically, return the number of requests that have happened in the inclusive range [t - 3000, t].
...
class RecentCounter {
  ...
  ping(time) {
    this.requests.push(time); //adds time into requests
    while (this.requests[0] < time - 3000) {
      this.requests.shift(); //keep removing the ones in the front that doesn't satisfy the range, since the ping time is guaranteed to only go up
    }
